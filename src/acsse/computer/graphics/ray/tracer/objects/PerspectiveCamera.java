...
public class PerspectiveCamera extends Camera {
	...
	public PerspectiveCamera(Vector origin, Vector target, Vector upguide, float angle, float aspectRation) {
		this.origin = origin;
		this.forwardVector = MathClass.normalize(MathClass.sub(target, origin));
		this.rightVector = MathClass.normalize(MathClass.crossProd(forwardVector, upguide));
		this.upVector = MathClass.crossProd(rightVector, forwardVector);
	...
	final public Ray produceRay(Vector2D point) {
		Vector dir = MathClass.add(forwardVector, MathClass.add(Transforms.scale(point.getU()*width, rightVector), Transforms.scale(point.getV() * height, upVector)));
	...
	@Override
	public Ray shootRay(Vector2D point) {
